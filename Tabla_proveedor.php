...
<div class="modal_inner">
  ...
  <div class="container">
	...
	<div class="form-group col-sm-12 col-md-12" hidden>
		...
		<input value="0" onkeypress="return restrictCharacters(this, event, digitsOnly);" class="form-control" type="Number" placeholder="id_proveedor" id="id_proveedor">
	</div>

			<div class="form-group col-sm-12 col-md-12">
				<label for="nombre">nombre</label>
				<textarea class="form-control" id="nombre" rows="3"></textarea>
			</div>
			<div class="form-group col-sm-12 col-md-12">
				<label for="telefono">telefono</label>
				<textarea class="form-control" id="telefono" rows="3"></textarea>
			</div>
		<div class="form-group p-1">
			<button class="btn btn-primary" data-target="modalFormulario" id="BotonGuardar" onclick="Guardar_Tabla_proveedor();">Guardar</button>
		</div>
		<div class="form-group p-1">
			<button class="btn btn-primary" data-target="modalFormulario" id="BotonModificar" onclick="Modificar_Tabla_proveedor();" disabled>Modificar</button>
		</div>
		<div class="form-group p-1">
			<button class="btn btn-primary" data-target="modalFormulario" id="BotonEliminar" onclick="Eliminar_Tabla_proveedor();" disabled>Eliminar</button>
		</div>
		<div class="form-group p-1">
...
?><?php
 ?><li class="nav-item"><a class="nav-link" href="./log_out.php">salir</a></li>
		...
		<main role="main" class="col-md-12 ml-sm-auto col-lg-12 pt-3 px-4" id="ContenedorMain">
		  ...
		  <div class="col-sm-12 col-md-12" style="padding-bottom: 5%;overflow-x: scroll;" id="ContenedorDomTabla_proveedor"></div>
...
var proteccionDeTRansaccion=false;
	...
	function Guardar_Tabla_proveedor(){
		...
		if(proteccionDeTRansaccion==false){
			$.post('./abc/Tabla_proveedor/Guardar_Tabla_proveedor.php',{
	...
	function Modificar_Tabla_proveedor(){
		if(document.getElementById('nombre').value.length>0&&document.getElementById('telefono').value.length>0){
		if(proteccionDeTRansaccion==false){
			$.post('./abc/Tabla_proveedor/Modificar_Tabla_proveedor.php',{
							id_proveedor:document.getElementById('id_proveedor').value,
			nombre:document.getElementById('nombre').value,
			telefono:document.getElementById('telefono').value
	...
	function Eliminar_Tabla_proveedor(){

		  let text = "Estas seguro que quieres eliminar el registro?";
		  if (confirm(text) == true) {
		    $.post('./abc/Tabla_proveedor/Eliminar_Tabla_proveedor.php',{
							id_proveedor:document.getElementById('id_proveedor').value
	...
	function DescongelarRegistro(){
		$.post('./abc/Tabla_proveedor/Descongelar_registro_Tabla_proveedor.php',{
						id_proveedor:document.getElementById('id_proveedor').value
	  ...
	  function SeleccionarRegistro(IdRegistro){
		$.post('./abc/Tabla_proveedor/extraer_registro_seleccionado.php',{
			id_proveedor:IdRegistro
		  }).done(function(data){
		  	if(data==0){
		  		$('#modalFormulario').toggleClass('-open');
	...
	function verificarExistenciaDelRegistro(nombreDeTabla,campoorigen,campovalor){
	  $.post('./abc/Tabla_proveedor/Verificar_existencia_Tabla_proveedor.php',{
  ...
  function Cargar_Tabla_proveedor() {
  	//Notify("Cargando registros espere.", null, null, 'info');
	  $.get("./abc/Tabla_proveedor/extraer_Tabla_proveedor.php",{
		}).done(function(data){
		  document.getElementById("ContenedorDomTabla_proveedor").innerHTML=data;
